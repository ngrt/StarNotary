...
it('can add the star name and star symbol properly', async() => {
    let instance = await StarNotary.deployed();
    // WHAT IS THE POINT OF CREATING A STAR TO TEST NAME AND SYMBOL CONTRACT...? 
    // Ignoring this task
    const symbol = await instance.symbol();
    const name = await instance.name();
    assert.equal(symbol, "STA");
    assert.equal(name, "StarAlliance");
});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let user1 = accounts[1];
    let starId = 9;
    const startName = 'awesome star';
    await instance.createStar(startName, starId, {from: user1});
    assert.equal(await instance.lookUptokenIdToStarInfo.call(starId), startName);
});